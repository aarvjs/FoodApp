...
export default function AdminKitchenOrdersPage() {
  ...
  // Filter orders by Branch & Search Query
  const filteredBranchOrders = orders.filter((ord) => {
    if (selectedBranchId !== "ALL" && ord.branchId !== selectedBranchId) {
      return false;
    }
    if (searchQuery.trim()) {
      ...
      const matchBranch = (ord.branchName || "").toLowerCase().includes(q);
      return matchNum || matchCust || matchPhone || matchBranch;
    }
    return true;
  });

  // Filter by Tab
  const finalOrders = filteredBranchOrders.filter((ord) => {
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-amber-950 rounded-3xl text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/20 text-amber-300 text-xs font-semibold rounded-full border border-amber-500/30 mb-2">
            <UtensilsCrossed className="w-3.5 h-3.5" /> All Branches Kitchen Queue
          </span>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Kitchen Order Tickets (KOT)</h1>
          <p className="text-slate-400 text-xs sm:text-sm mt-1 max-w-xl">
            Real-time live kitchen display console across all branches with full combo & customization breakdowns.
          </p>
        </div>

        {/* Branch Filter Selector */}
        <div className="flex items-center gap-2 bg-slate-800/90 border border-slate-700/80 p-2 rounded-2xl shrink-0">
          <Filter className="w-4 h-4 text-amber-400 ml-1" />
          <span className="text-xs font-bold text-slate-300">Branch:</span>
          <select
          ...
          >
            <option value="ALL">All Network Branches ({branches.length})</option>
            {branches.map((b) => (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
          {[
            ...
            { id: "ALL", label: "All Orders", icon: ShoppingBag, color: "bg-slate-900 text-white" },
          ].map((tab) => {
            const isActive = activeTab === tab.id;
            const Icon = tab.icon;
            const count = filteredBranchOrders.filter((ord) => {
              const st = (ord.status || "").toUpperCase();
      ...
      {finalOrders.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-3xl border border-slate-200/80 shadow-xs space-y-3">
          ...
          <h3 className="font-bold text-slate-900 text-base">No Kitchen Orders Found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            No orders match the selected status filter or branch criteria. New incoming customer orders will appear automatically.
          </p>
        </div>
      ) : (
      ...
      {kotOrder && (
        <KitchenTicketModal
          order={kotOrder}
          branch={branches.find((b) => b.id === kotOrder.branchId)}
      ...
      {invoiceOrder && (
        <OrderInvoiceModal
          order={invoiceOrder}
          branch={branches.find((b) => b.id === invoiceOrder.branchId)}
